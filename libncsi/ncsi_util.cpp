...
CallBack dataCallBack = [](struct nl_msg* msg, void* arg) {
  ...
  /*  ncsi_data_len = nla_len(tb[NCSI_ATTR_DATA]);
    ...
    printf("3.5 \n");
    printf(" LEn = %d\n", nla_len(tb[NCSI_ATTR_DATA]));
    printf("3.6 \n");
    resp_data.recv_len = nla_len(tb[NCSI_ATTR_DATA]);
    printf("Callback 4 \n");
    memcpy(resp_data.recv_ncsi_data, (nla_data(tb[NCSI_ATTR_DATA])), resp_data.recv_len);
    printf("Callback 5 \n");
   ...
   /* std::cerr << "NC-SI Response Payload length = "<< ncsi_data_len <<"\n";
    ...
    std::cerr << "NC-SI Response Payload length = "<< resp_data.recv_len <<"\n";
    std::cerr << "Response Payload:\n0: ";
...
int applyCmd(uint8_t paddingBytes, int ifindex, int cmd, int package = DEFAULT_VALUE,
             ...
             short payload_len = DEFAULT_VALUE, uint8_t* payload = nullptr)
{
    ...
    {
        pData = (uint8_t*)calloc(1, sizeof(struct ncsi_pkt_hdr) + payload_len + paddingBytes);
        if (!pData)
        {
        ...
        printf("Sizeof ncsi hdr : %d\n", sizeof(struct ncsi_pkt_hdr));
        printf("Payload len : %d\n", payload_len + paddingBytes);

        if(paddingBytes > 0)
        {    
                pCtrlPktPayload = pData + sizeof(struct ncsi_pkt_hdr) + payload_len;
		pCtrlPktPayload[payload_len] = 0;
		pCtrlPktPayload[payload_len + 1] = 0;
		pCtrlPktPayload[payload_len + 2] = 0;

		printf("Payload : ");
		for(int i=0; i<(payload_len + paddingBytes); i++)
		{
			printf("%x\t", pCtrlPktPayload[i]);
		}
		printf("\n");     
        }	

        ret = nla_put(msg.get(), NCSI_ATTR_DATA,
                      sizeof(struct ncsi_pkt_hdr) + payload_len + paddingBytes, (void*)pData);
        if (ret < 0)
        {
...
int setChannel(int ifindex, int package, int channel)
{
    ...
    return internal::applyCmd(0, ifindex, ncsi_nl_commands::NCSI_CMD_SET_INTERFACE,
                              package, channel);
}

int sendCommand(int ifindex, int package, int channel, int opcode,
                short payload_len, uint8_t* payload, uint8_t extra_bits)
{
    ...
    return internal::applyCmd(extra_bits, ifindex, ncsi_nl_commands::NCSI_CMD_SEND_CMD,
...
int clearInterface(int ifindex)
{
    std::cout << "ClearInterface , IFINDEX :" << std::hex << ifindex
              << std::endl;
    return internal::applyCmd(0, ifindex,
...
int getInfo(int ifindex, int package)
{
    ...
    if (package == DEFAULT_VALUE)
    {
        return internal::applyCmd(0, ifindex, ncsi_nl_commands::NCSI_CMD_PKG_INFO,
